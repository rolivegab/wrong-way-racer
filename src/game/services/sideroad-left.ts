...
const finalPosition = {
  position: {
    x: applicationService.screen.width * -0.9,
...
const load = () => {
  const texture = Assets.get(TEXTURE_KEYS.SIDEROAD_LEFT);
  state.sprite.texture = texture;
  gameService.addActor(ACTOR_KEYS.SIDEROAD_LEFT, sideRoadLeftService);
  initialize();
};
...
export const sideRoadLeftService = extendObservable(state, {
  load,
  onTick,
}) satisfies Actor;
