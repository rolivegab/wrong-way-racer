...
const initialPosition = {
  position: {
    x: applicationService.screen.width * 0.45,
...
const finalPosition = {
  position: {
    x: applicationService.screen.width * -0.25,
    y: applicationService.screen.height * 1.2,
  },
  width: 700,
};
...
const load = () => {
  const texture = Assets.get(TEXTURE_KEYS.MOUNTAIN_LEFT);
  state.sprite.texture = texture;
  gameService.addActor(ACTOR_KEYS.MOUNTAIN_LEFT, mountainLeftService);
  gameService.addToStage(state.sprite);
  initialize();
};

const onTick: Actor["onTick"] = (delta, elapsedMS) => {
...
export const mountainLeftService = extendObservable(state, {
  load,
  onTick,
}) satisfies Actor;
